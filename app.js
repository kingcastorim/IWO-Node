...
if (gui.App.fullArgv.indexOf('--reset') !== -1) {
	...
	fs.unlinkSync(path.join(data_path, 'data/cached_shows.db'), function (err) {
		if (err) {
			throw err;
		}
	});
	fs.unlinkSync(path.join(data_path, 'data/cached_movies.db'), function (err) {
		if (err) {
			throw err;
		}
	});
	fs.unlinkSync(path.join(data_path, 'data/cached_shows_links.db'), function (err) {
		if (err) {
			throw err;
		}
	});
