...
//cached movies and shows
db.cached_movies = new Datastore({
	filename: path.join(data_path, 'data/cached_movies.db'),
	autoload: true
});
db.cached_shows = new Datastore({
	filename: path.join(data_path, 'data/cached_shows.db'),
	autoload: true
});
db.cached_shows_links = new Datastore({
	filename: path.join(data_path, 'data/cached_shows_links.db'),
	autoload: true
});
...
promisifyDatastore(db.cached_movies);
promisifyDatastore(db.cached_shows);
promisifyDatastore(db.cached_shows_links);
...
db.cached_movies.ensureIndex({
	fieldName: 'imdb_id',
	unique: true
});
db.cached_shows.ensureIndex({
	fieldName: 'imdb_id',
	unique: true
});
db.cached_shows_links.ensureIndex({
	fieldName: 'e_id',
	unique: true
});

db.movies.removeIndex('imdb_id');
db.tvshows.removeIndex('imdb_id');
db.tvshows.removeIndex('tvdb_id');
db.bookmarks.removeIndex('imdb_id');
db.cached_movies.removeIndex('imdb_id');
db.cached_shows.removeIndex('imdb_id');
db.cached_shows.removeIndex('e_id');
...
var Database = {
	...
	/****************************************************************/
	addMovieCache: function (data) {
		return db.cached_movies.insert(data);
	},
	
	deleteMovieCache: function (imdb_id) {
		return db.cached_movies.remove({
			imdb_id: imdb_id
		});
	},
	
	updateMovieCache:function (data) {
		return db.cached_movies.update({
			imdb_id: data.imdb_id
		}, data);
	},
	
	getMovieCache: function (imdb_id) {
		return promisifyDb(db.cached_movies.findOne({
			imdb_id: imdb_id
		}));
	},
	
	//{keyword: title}
	getMovieCacheByFilter: function (data) {
		if (data.title === undefined || data.title === "")
			return this.getAllMovieCache();
		else
			return promisifyDb(db.cached_movies.find({
				title: data.title
			}));
	},
	
	getShowCacheByFilter: function (data) {
		if (data.title === undefined || data.title === "")
			return this.getAllShowCache();
		else
			return promisifyDb(db.cached_shows.find({
				title: data.title
			}));
	},
	
	getAllMovieCache: function () {
		return promisifyDb(db.cached_movies.find({}));
	},
	
	addShowCache: function (data) {
		return db.cached_shows.insert(data);
	},
	
	updateShowCache:function (data) {
		return db.cached_shows.update({
			imdb_id: data.imdb_id
		}, data);
	},
	
	deleteShowCache: function (imdb_id) {
		return db.cached_shows.remove({
			imdb_id: imdb_id
		});
	},
	
	getShowCache: function (imdb_id) {
		return promisifyDb(db.cached_shows.findOne({
			imdb_id: imdb_id
		}));
	},
	
	getAllShowCache: function () {
		return promisifyDb(db.cached_shows.find({}));
	},
	
	
	addShowLinkCache: function (data) {
		return db.cached_shows_links.insert(data);
	},
	
	updateShowLinkCache:function (data) {
		return db.cached_shows_links.update({
			e_id: data.e_id
		}, data);
	},
	
	deleteShowLinkCache: function (e_id) {
		return db.cached_shows_links.remove({
			e_id: e_id
		});
	},
	
	getShowLinkCache: function (e_id) {
		return promisifyDb(db.cached_shows_links.findOne({
			e_id: e_id
		}));
	},
	
	getShowLinkCacheById: function (id) {
		return promisifyDb(db.cached_shows_links.findOne({
			id: id
		}));
	},
	...
	deleteDatabases: function () {
		...
		fs.unlinkSync(path.join(data_path, 'data/cached_movies.db'));
		
		fs.unlinkSync(path.join(data_path, 'data/cached_shows.db'));
		
		fs.unlinkSync(path.join(data_path, 'data/cached_shows_links.db'));
